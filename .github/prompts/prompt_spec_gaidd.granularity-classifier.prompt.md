---
name: "Paso 1: Clasificación de Granularidad del Requerimiento"
...
2. Cargar {project-root}/.github/docs/context/reglas-de-oro.md y asegurarse de seguirlas estrictamente durante todo el proceso de clasificación.

3. Determinar si el artefacto a evaluar ya fue proporcionado en el contexto de la conversación:
...
4. Con el artefacto en contexto, ejecutar la clasificación del tipo de artefacto aplicando los siguientes criterios en orden:
   ...
   - Está formulado desde la perspectiva del usuario o actor de negocio describiendo una necesidad
   - Utiliza formato ágil explícito con criterios de aceptación en formato BDD (Dado-Cuando-Entonces) o similar
   - Contiene campos típicos de historias de usuario: puntos de historia, prioridad de backlog, sprint asignado, épica padre
   ...
   - Es una especificación funcional o no funcional formal con lenguaje orientado al sistema
   - Utiliza formato de especificación técnica (IEEE 830, ISO/IEC/IEEE 29148 o similar)
   ...
   Si el artefacto NO encaja claramente en ninguna categoría o presenta ambigüedad:
   - Comunicar al usuario los indicadores encontrados para ambas categorías
   - Solicitar al usuario que confirme el tipo de artefacto antes de continuar
   - DETENERSE y ESPERAR confirmación

5. Comunicar al usuario en {communication_language} el tipo detectado con una justificación breve (máximo 2-3 oraciones indicando qué indicadores se identificaron) y solicitar confirmación explícita antes de continuar:
   - SI el usuario confirma → continuar al paso 5
   - SI el usuario corrige → aceptar la corrección y continuar al paso 5 con el tipo corregido
   - DETENERSE y ESPERAR confirmación

6. Según el tipo de artefacto confirmado, cargar y ejecutar el agente correspondiente:
   - **Historia de Usuario** →
   ...
   IMPORTANTE: Al transferir el control al agente, el artefacto ya está en contexto — NO solicitar el artefacto nuevamente. Indicar al agente que el artefacto ya fue proporcionado y clasificado.
